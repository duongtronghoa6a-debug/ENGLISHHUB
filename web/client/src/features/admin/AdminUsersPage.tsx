...
const AdminUsersPage = () => {
    ...
    const stats = {
        total: pagination.total || users.length,  // Use API total, not just current page
    ...
    const handleExportExcel = () => {
        const exportData = filteredUsers.map(user => ({
            'Họ tên': user.profile?.full_name || 'Chưa cập nhật',
            'Email': user.email,
            'Số điện thoại': user.profile?.phone || 'N/A',
                                    ...
                                    <tr key={user.id} className={`${isDarkMode ? 'hover:bg-white/5' : 'hover:bg-gray-50'} transition-colors group`}>
                                        <td className="p-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div className="font-bold">{user.profile?.full_name || 'Chưa cập nhật'}</div>
                                        ...
                                        <td className="p-4 text-right">
                                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button className="p-2 hover:bg-blue-500/10 text-blue-500 rounded-lg">
                                                    <Edit size={16} />
                                                </button>
