...
exports.getAllCourses = async (req, res, next) => {
    try {
        const { page = 1, limit = 12, is_free, level, category, is_published, search } = req.query;
        ...
        if (search && search.trim()) {
            where[db.Sequelize.Op.or] = [
                { title: { [db.Sequelize.Op.iLike]: `%${search.trim()}%` } },
                { description: { [db.Sequelize.Op.iLike]: `%${search.trim()}%` } }
            ];
        }
